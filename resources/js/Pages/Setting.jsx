...
const Setting = () => {
	...
	const [security, setSecurity] = useState({current_password: '', new_password: '', new_password_confirmation: ''});
	...
	useEffect(() => {
		if (auth.user?.image) {
			setImagePreview(`/storage/${auth.user.image}`);
		} else {
	...
	// Security content component (unchanged)
	...
	return (
		<div className="flex h-screen bg-gray-50">
			...
			{/* Settings Sidebar */}
			<div className={`fixed inset-y-0 left-0 z-30 w-72 bg-white border-r border-gray-200 transform transition duration-300 ease-in-out lg:static lg:translate-x-0 ${
								...
								{section.items.map((item) => (
									<button 
										...
										className={`flex items-center w-full px-6 py-2.5 text-sm transition-colors ${
											activeSection === item.id ? 'text-indigo-600 bg-indigo-50 border-r-2 border-indigo-600' : 'text-gray-700 hover:bg-gray-50'
										}`}
									>
