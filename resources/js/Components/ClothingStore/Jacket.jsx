...
import { ShoppingCart, X, Filter, ChevronDown, ChevronUp, Star, StarHalf, ChevronLeft, ChevronRight, Eye } from 'lucide-react';
...
const Jacket = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter products to only include jackets
        const jacketProducts = response.data.data.filter(product => 
          product.category && product.category.toLowerCase().includes('jacket')
        );
        
        // Normalize product data
        const normalizedProducts = jacketProducts.map(product => ({
          ...product,
          name: product.product_name,
          label: product.product_name,
          image_url: product.image_url || '/images/placeholder-jacket.jpg',
  ...
  // Filter products based on jacket subcategories and sidebar filters
...
const ProductCard = ({ product, onSelect, onAddToCart, renderRating, cart }) => {
  return (
...
src={`${imagepath}/${product.images[0].image_path}`}   
       alt={product.name}
        ...
        <button 
          ...
          className="absolute bottom-4 left-4 right-4 bg-black text-white py-3 text-sm tracking-wide opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 flex items-center justify-center gap-2"
        >
...
// Product Details Modal Component with Multiple Images
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <div className="sticky top-0 bg-white border-b border-neutral-200 p-6 flex justify-end z-10">
          <button 
            onClick={onClose}
...
src={`${imagepath}/${product.images.image_path}`}       
          ...
          <div className="space-y-6">
            ...
            <p className="text-neutral-600 leading-relaxed">
              {product.description || `This is a detailed description of the ${product.name}. It features high-quality materials and craftsmanship perfect for any occasion. The fabric is comfortable and durable, making it an excellent addition to your wardrobe.`}
            </p>
            ...
            <div className="border-t border-neutral-200 pt-6">
              <h3 className="text-sm font-medium text-neutral-900 mb-3">Product Features</h3>
